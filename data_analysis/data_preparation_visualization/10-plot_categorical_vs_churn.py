...
def plot_categorical_vs_churn(df, col):
    ...
    """
    # churn rate/category = no of yes row in each group / total row in that group
    # group means e.g. under gender col, male and female are two separate groups
    # mean() is used to compute proportion of yes values in each group 
    ...
    # Male: ~26.2% = Female: ~26.95% ,showing no signal for the column.
    # After running further analysis we will validate the removal of this column.
    # The figure shows a churn rate for PhoneService:
    # No: ~25% and Yes: ~26.74% Showing no signal for the column, indicating a very
    # small difference and suggesting that this feature shows no
    # strong signal for predicting churn.
    return None
